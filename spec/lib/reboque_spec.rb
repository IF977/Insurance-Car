...
describe Reboque do
   ...
   # invalid_input= "[^A-Za-z0-9 ]"
    ...
    it "O codigo é válido?" do 
    	#verifica se tem espaço e letras que compõe o codigo
    ...
    it "A distancia é válida?" do
        #algoritmo para validar kilometragem
        expect(obj.distancia).to match(regex_num)
        expect(obj.distancia).not_to be_empty
        expect(Integer(obj.distancia)).not_to eq(0)
    end
    it "A Origem é valida?" do
        #algoritmo para validar o relatorio
        expect(obj.origem).not_to be_empty
        expect(obj.origem).to match(regex_text)
    end
    it "O Destino é valido?" do
        #algoritmo para validar o relatorio
        expect(obj.destino).not_to be_empty
        expect(obj.destino).to match(regex_text)
    end
end
