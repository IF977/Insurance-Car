...
describe Cliente do
    ...
    it "O nome é válido?" do
        expect(obj.nome).to match(valid_input) and not_to match(regex_num) #verifica se não tem números, verifica se tem espaço e letras
    end
    
    it "endereco é valido?" do
        expect(obj.endereco).to match(regex_endereco)
        expect(obj.endereco).not_to match(" ")
        expect(obj.endereco).not_to be_empty
        expect(obj.endereco).to be_an_instance_of(String)
    end
    
    it "telefone só tem numeros?" do
        expect(obj.telefone).to match(regex_number)
